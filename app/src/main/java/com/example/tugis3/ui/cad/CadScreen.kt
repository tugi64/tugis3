...
fun CadScreen(vm: CadViewModel, snackbarHost: SnackbarHostState) {
    ...
    val snapTolerance by vm.snapTolerancePx.collectAsState()
    val gridVisible by vm.gridVisible.collectAsState()
    val stake by vm.stakeout.collectAsState() // yeni stakeout state
    ...
    var syncBounds by remember { mutableStateOf<Bounds?>(null) }
    ...
    Column(Modifier.fillMaxSize()) {
        ...
        Row(Modifier.fillMaxWidth().padding(horizontal = 8.dp, vertical = 4.dp), horizontalArrangement = Arrangement.spacedBy(6.dp)) {
            ...
            AssistChip(onClick = {
                mapTheme = when(mapTheme){ MapTheme.AUTO -> MapTheme.LIGHT; MapTheme.LIGHT -> MapTheme.DARK; MapTheme.DARK -> MapTheme.AUTO }
            }, label = { Text("Tema: ${mapTheme.label}") })
        }
        // Durum kartı
        Card(
        ...
        Box(Modifier.weight(1f).fillMaxWidth()) {
            ...
            val activeBounds = syncBounds ?: bounds
            // Harita
            if (showMap) {
                // Kamera takip
                ...
                GoogleMap(
                    ...
                    entities.forEach { e ->
                        ...
                        when(e){
                            ...
                            is CadPolygon -> {
                                val ring = e.rings.first().mapNotNull { p -> vm.localToLatLon(p.x,p.y)?.let { LatLng(it.first,it.second) } }
                                if (ring.size >= 3) Polygon(points = ring, fillColor = if (isSelected) Color(0x44FFFF00) else Color(0x3300FF00), strokeColor = if (isSelected) Color.Yellow else Color.Green, strokeWidth = if (isSelected) 6f else 2f)
                            }
                            is CadCircle -> {
                ...
                Box(Modifier.matchParentSize()) {
                    if (entities.isNotEmpty()) {
                        CadCanvas(
                            ...
                            snapTolerancePx = snapTolerance.toFloat(),
                    ...
                    // Seçili entity bilgi paneli
                    if (selectedEntity != null) {
                        val se = selectedEntity // local reference to avoid smart cast issues
                        if (se != null) SelectedEntityInfo(se, modifier = Modifier.align(androidx.compose.ui.Alignment.TopEnd).padding(8.dp))
                    }
                    // Stakeout Panel
                    if (stake.hasFix) {
                        Card(
...
@Composable
private fun SelectedEntityInfo(entity: CadEntity, modifier: Modifier = Modifier) {
    val info = remember(entity) {
        val base = when(entity) {
            ...
            is CadPolygon -> {
                ...
                val area = CadViewModel.polygonAreaOf(outer)
                "Polygon | rings=${entity.rings.size} ptsOuter=${outer.size} A=${"%.3f".format(area)} P=${"%.3f".format(perim)}"
            }
            is CadCircle -> {
...
private fun computeBounds(entities: List<CadEntity>): Bounds {
    ...
    entities.forEach { e ->
        when (e) {
            ...
            is CadCircle -> {
                acc(Point(e.center.x - e.radius, e.center.y - e.radius))
                acc(Point(e.center.x + e.radius, e.center.y + e.radius))
            }
            is CadArc -> {
                acc(Point(e.center.x - e.radius, e.center.y - e.radius))
...
private fun CadCanvas(
    ...
    Canvas(modifier = Modifier
        ...
        .pointerInput(entities, bounds, picked, snapEnabled, selectionMode, snapTolerancePx, canvasSize) {
...
suspend fun exportCadAndMap(context: android.content.Context, view: android.view.View) {
    // 1. ComposeView'dan bitmap al
    val bitmap = getBitmapFromView(view)
    // 2. (Opsiyonel) GoogleMap snapshot alınacaksa MapView interop gerekir
    // 3. PNG olarak kaydet
